...
if ( !class_exists( 'RatingWidgetPlugin' ) ) {
    ...
    class RatingWidgetPlugin
    {
        ...
        function init_fs_hooks()
        {
            ...
            $this->fs->add_action(
                ...
                array( &$this, 'connect_account' ),
                10,
                2
            );
            $this->fs->add_action(
                'after_premium_version_activation',
                array( &$this, 'after_premium_version_activation_hook' ),
                10,
                2
            );
        ...
        private function setup_dashboard_actions()
        {
            ...
            add_action(
                'updated_post_meta',
                array( &$this, 'PurgePostFeaturedImageTransient' ),
                10,
                4
            );
        ...
        function five_star_wp_rate_action()
        {
            ...
            if ( 'do-rate' === $rate_action ) {
                ...
                if ( 10 === $min_votes_trigger ) {
                    $min_votes_trigger = 100;
                } else {
                    
                    if ( 100 === $min_votes_trigger ) {
        ...
        function prevent_referrer_policy_override()
        {
            ...
            if ( $this->_inDashboard ) {
                ...
                $is_settings_page_with_rating = $page === $this->GetMenuSlug() || $page === $this->GetMenuSlug( 'woocommerce' ) || $page === $this->GetMenuSlug( 'bbpress' ) || $page === $this->GetMenuSlug( 'buddypress' );
                if ( !$is_settings_page_with_rating ) {
                    return;
                }
            } else {
                global  $pagenow ;
                if ( !$this->admin_page_has_editor() && !('comment.php' === $pagenow && !$this->is_comment_review_mode()) ) {
                    return;
                }
            }
        ...
        function add_rating_before_comment_form_submit_button( $comment_form_defaults )
        {
            ...
            if ( isset( $comment_form_defaults['submit_field'] ) ) {
                ...
                if ( isset( $comment_form_defaults['comment_notes_after'] ) ) {
                    if ( RWLogger::IsOn() ) {
        ...
        function get_comment_form_rating_html()
        {
            ...
            return $html = '<p>' . $this->GetRatingHtml(
                $urid,
                'comment',
                false,
                '',
        ...
        function comment_review_submission_errors( $api_request_info )
        {
            ...
            if ( is_wp_error( $api_response ) ) {
                ...
                if ( 200 !== wp_remote_retrieve_response_code( $api_response ) ) {
                    ...
                    $errors['invalid_response_code'] = array( 'Invalid response code' );
                } else {
                    ...
                    if ( !is_object( $json_obj ) ) {
                        $errors['invalid_response_string'] = array( 'Invalid response string' );
                    } else {
                        if ( !$json_obj->success ) {
                            $errors['api_request_failed'] = array( $json_obj->msg );
                        }
                    }
        ...
        function add_comment_rating_metabox()
        {
            ...
            if ( current_user_can( 'manage_options' ) && !$this->is_comment_review_mode() ) {
                add_meta_box(
                    'rw-comment-meta-box',
                    WP_RW__NAME,
                    array( &$this, 'show_comment_rating_metabox' ),
                    'comment',
                    'normal'
                );
            }
        ...
        {
            $this->Notice( 'Hmm... it looks like your license remained the same. If you did upgrade, it\'s probably an issue on our side (sorry). Please contact us <a href="' . rw_get_site_url( '/contact/?' . http_build_query( array(
                ...
                'platform' => 'wordpress',
                'message'  => 'I\'ve upgraded my account but when I try to Sync the License in my WordPress Dashboard -> Ratings -> Account, the license remains the same.' . "\n" . 'Your Upgraded Plan: [REPLACE WITH PLAN NAME]' . "\n" . 'Your PayPal Email: [REPLACE WITH PAYPAL ADDRESS]',
        ...
        function five_star_wp_rate_notice()
        {
            ...
            if ( !isset( $response->error ) ) {
                ...
                if ( $votes >= $min_votes_trigger ) {
                    ...
                    $params = array(
                        'min_votes_trigger' => $min_votes_trigger,
                    );
                    ob_start();
        ...
        protected function LoadDefaultOptions()
        {
            ...
            $top_left = (object) array(
                'ver' => 'top',
                'hor' => 'left',
            );
            $bottom_left = (object) array(
                'ver' => 'bottom',
                'hor' => 'left',
            );
        ...
        function RemoteCall( $pPage, &$pData, $pExpiration = false )
        {
            ...
            $rw_ret_obj = wp_remote_post( WP_RW__ADDRESS . "/{$pPage}", array(
                'body' => $pData,
            ) );
        ...
        function InitScriptsAndStyles()
        {
            ...
            rw_enqueue_style( 'rw_fonts', add_query_arg( array(
                'family' => 'Noto+Sans:400,700,400italic,700italic',
            ), WP_RW__PROTOCOL . '://fonts.googleapis.com/css' ) );
            ...
            if ( !$this->account->is_registered() ) {
                ...
                if ( 'rating-widget-addons' === $_GET['page'] ) {
                    ...
                    if ( 'rating-widget-affiliation' === $_GET['page'] ) {
                        ...
                        if ( 'rating-widget-workflows' !== $_GET['page'] ) {
        ...
        function get_multirating_options_by_class( $rclass )
        {
            ...
            if ( !isset( $multirating_settings_list->{$rclass} ) || isset( $multirating_settings_list->{$rclass} ) && !is_array( $multirating_settings_list->{$rclass}->criteria ) ) {
        ...
        function SetupMenuItems()
        {
            ...
            if ( false === is_active_widget(
                false,
                false,
                strtolower( 'RatingWidgetPlugin_TopRatedWidget' ),
                true
            ) ) {
        ...
        public static function _isValidPCId( $pDeviceID )
        {
            ...
            if ( $pDeviceID[8] != "-" || $pDeviceID[13] != "-" || $pDeviceID[18] != "-" || $pDeviceID[23] != "-" ) {
                return false;
            }
            for ( $i = 0 ;  $i < 36 ;  $i++ ) {
                ...
                if ( $code < 48 || $code > 70 || $code > 57 && $code < 65 ) {
                    return false;
                }
            }
            return true;
        ...
        function SettingsPage()
        {
            ...
            if ( isset( $_POST[$rw_form_hidden_field_name] ) && $_POST[$rw_form_hidden_field_name] == 'Y' && !empty($_POST['rw_settings_nonce']) && wp_verify_nonce( $_POST['rw_settings_nonce'], basename( WP_RW__PLUGIN_FILE_FULL ) ) ) {
        ...
        function rw_validate_category_availability( $pId, $pClass )
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( "rw_validate_category_availability", $params );
            }
            ...
            if ( !isset( $this->categories_list->{$pClass} ) || empty($this->categories_list->{$pClass}) ) {
                return true;
            }
            // Alias.
        ...
        function rw_validate_visibility( $pId, $pClasses = false )
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( "rw_validate_visibility", $params );
            }
            ...
            foreach ( $pClasses as $class ) {
                ...
                if ( $visibility_list->selected === WP_RW__VISIBILITY_EXCLUDE && in_array( $pId, $visibility_list->exclude ) || $visibility_list->selected === WP_RW__VISIBILITY_INCLUDE && !in_array( $pId, $visibility_list->include ) ) {
                    return false;
                }
            }
            return true;
        ...
        function is_rating_readonly( $post_id, $class )
        {
            ...
            if ( !in_array( $post_id, $readonly_list->active ) && !in_array( $post_id, $readonly_list->readonly ) ) {
                $options = $this->GetOption( $option_name );
                return ( isset( $options->readOnly ) ? $options->readOnly : false );
            }
        ...
        function add_to_visibility_list( $pId, $pClasses, $pIsVisible = true )
        {
            ...
            if ( is_string( $pClasses ) ) {
                ...
                if ( !is_array( $pClasses ) || 0 == count( $pClasses ) ) {
                    return;
                }
            }
        ...
        function rw_validate_availability( $pClass )
        {
            ...
            if ( true === $this->is_user_logged_in || !isset( $this->availability_list->{$pClass} ) ) {
        ...
        function IsVisibleRating(
            $pElementID,
            $pClass,
            $pValidateCategory = true,
            $pValidateVisibility = true
        )
        {
        ...
        function GetPostImage( $pPost, $pExpiration = false )
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( "GetPostImage", $params );
            }
        ...
        function add_post_rating( $content, $rclass )
        {
            ...
            $ratingHtml = $this->EmbedRatingIfVisibleByPost(
                $post,
                $rclass,
                true,
                $this->post_align->hor,
            ...
            return ( 'top' === $this->post_align->ver ? $ratingHtml . $content : $content . $ratingHtml );
        }
        ...
        function AddCommentRating( $content )
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( 'AddCommentRating', $params );
            }
            ...
            return ( 'top' === $this->comment_align->ver ? $ratingHtml . $content : $content . $ratingHtml );
        }
         ...
         */
        private function GetRatingHtml(
            $pUrid,
            $pElementClass,
            $pAddSchema = false,
            $pTitle = "",
        ...
        function BuddyPressBeforeActivityLoop( $has_activities )
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( "BuddyPressBeforeActivityLoop", $params );
            }
        ...
        function rw_display_activity_comment_rating( $comment_content )
        {
            ...
            if ( isset( $_POST['action'] ) && 'new_activity_comment' == $_POST['action'] ) {
                ...
                $this->current_comment->children = array(
                    $current_comment->id => $current_comment,
                );
            }
        ...
        function InitBuddyPress()
        {
            ...
            if ( isset( $_POST['action'] ) && isset( $_POST['cookie'] ) && 0 === strpos( $_POST['cookie'], 'bp-activity' ) ) {
        ...
        function IsBBPressInstalled()
        {
            ...
            // && (!function_exists('is_plugin_active') || is_plugin_active(WP_RW__BP_CORE_FILE));
        }
        ...
        function SetupBBPress()
        {
            ...
            define( 'WP_RW__BBP_INSTALLED', false );
            if ( WP_RW__BBP_INSTALLED && !is_admin() ) {
        ...
        function SetupBBPressTopicActions( $has_replies )
        {
            ...
            if ( false !== $this->GetRatingAlignByType( WP_RW__USERS_ALIGN ) && !$this->IsHiddenRatingByType( 'user' ) ) {
                ...
                add_filter(
                    ...
                    array( &$this, 'AddBBPressForumThreadUserRating' ),
                    10,
                    2
                );
            }
        ...
        function AddBBPressBottomRating( $content, $reply_id = 0 )
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( 'AddBBPressBottomRating', $params );
            }
        ...
        function AddBBPressTopCenterRating()
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( 'AddBBPressTopCenterRating', $params );
            }
        ...
        function AddBBPressTopLeftOrRightRating()
        {
            ...
            $ratingHtml = $this->EmbedRatingIfVisibleByPost(
                ...
                $class,
                false,
                'f' . $this->forum_post_align->hor,
                'display: inline; margin-' . (( 'left' === $this->forum_post_align->hor ? 'right' : 'left' )) . ': 10px;'
            );
            echo  $ratingHtml ;
        }
        ...
        function rw_before_forum_loop( $has_posts )
        {
            ...
            if ( RWLogger::IsOn() ) {
                $params = func_get_args();
                RWLogger::LogEnterence( "rw_before_forum_loop", $params );
            }
        ...
        function rw_attach_rating_js( $pElement = false )
        {
            ...
            $rw_settings = array(
                ...
                "user-activity-update"  => array(
                "options" => WP_RW__USERS_ACTIVITY_UPDATES_OPTIONS,
            ),
                "user-activity-comment" => array(
                "options" => WP_RW__USERS_ACTIVITY_COMMENTS_OPTIONS,
            ),
                "user-forum-post"       => array(
                "options" => WP_RW__USERS_FORUM_POSTS_OPTIONS,
            ),
            );
            foreach ( $this->_extensions as $ext ) {
                ...
                foreach ( $ext_settings as $type => $options ) {
                    $rw_settings[$options['class']] = array(
        ...
        function test_footer_init()
        {
            ...
            if ( isset( $_GET['test-footer'] ) ) {
                add_action( 'wp_footer', array( &$this, 'test_footer' ), 99999 );
            }
            // Some obscene priority, make sure we run last
        }
        ...
        function test_head_footer_notices()
        {
            ...
            echo  '<div class="updated highlight"><p><strong>' . esc_html( 'If the Rating-Widget\'s ratings don\'t show up on your blog it\'s probably because your active theme is missing the call to <?php wp_footer(); ?> which should appear directly before </body>.' ) . '</strong> ' . 'For more details check out our <a href="' . WP_RW__ADDRESS . '/faq/" target="_blank">FAQ</a>.</p></div>' ;
        }
        ...
        function SavePostData( $post_id )
        {
            ...
            if ( in_array( $_POST['post_type'], array(
                'post',
                'page',
                'product',
                'topic',
        ...
        function PurgePostFeaturedImageTransient(
            $meta_id = 0,
            $post_id = 0,
            $meta_key = '',
            $_meta_value = ''
        ...
        function GetPostExcerpt( $pPost, $pWords = 15 )
        {
            ...
            return ( mb_strlen( $strippedContent ) !== mb_strlen( $excerpt ) ? $excerpt . "..." : $strippedContent );
        }
        ...
        )
        {
            ...
            if ( false !== ($hor_align || $custom_style) ) {
                $html = '<div' . (( false !== $custom_style ? ' style="' . $custom_style . '"' : '' )) . (( false !== $hor_align ? ' class="rw-' . $hor_align . '"' : '' )) . '>' . $html . '</div>';
            }
            return $html;
        }
        
        function EmbedRatingIfVisible(
            $pElementID,
            $pOwnerID,
            $pTitle,
            $pPermalink,
        ...
        function EmbedRatingIfVisibleByPost(
            $pPost,
            $pClass = 'blog-post',
            $pAddSchema = false,
            $pHorAlign = false,
        ...
        function EmbedRatingIfVisibleByUser(
            $pUser,
            $pClass = 'user',
            $pCustomStyle = false,
            $pOptions = array()
    ...
    function rw_migration_to_freemius()
    {
        ...
        rw_fs()->setup_account( new FS_User( $result->user ), new FS_Site( $result->install ), false );
        return true;
    }
    ...
    function rw_reset_account()
    {
        ...
        if ( !$rw_api->test() ) {
            ...
            $rw_account->set_site(
                $prev_site_id,
                $prev_site_public_key,
                $prev_site_secret_key,
                false
            );
            // Reload API with new account details.
            $rw_api->reload();
